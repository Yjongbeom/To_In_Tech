import gi
gi.require_version("Gtk", "3.0")
from gi.repository import Gtk, Gdk, GLib
import RPi.GPIO as GPIO
import board
import busio
import adafruit_ads1x15.ads1115 as ADS
from adafruit_ads1x15.analog_in import AnalogIn
from adafruit_ina219 import INA219
import serial
import time
import threading
import os
from datetime import datetime
import subprocess
import gc

BASE_LOG_DIR = os.path.join(os.path.expanduser('~'), 'Desktop', '로그파일')
LOG_DIRECTORY = ""
log_file_path = ""
last_log_hour = -1

def setup_logging():
    global log_file_path, LOG_DIRECTORY, last_log_hour
    try:
        today_str = time.strftime("%Y-%m-%d")
        LOG_DIRECTORY = os.path.join(BASE_LOG_DIR, today_str)
        if not os.path.isdir(LOG_DIRECTORY):
            os.makedirs(LOG_DIRECTORY)
        current_hour = datetime.now().hour
        last_log_hour = current_hour
        timestamp = time.strftime("%Y-%m-%d_%H-00-00")
        log_file_path = os.path.join(LOG_DIRECTORY, f"{timestamp}_system.log")
        print(f"시스템 로그 파일이 '{log_file_path}'에 생성됩니다.")
    except Exception as e:
        print(f"로그 파일 설정 오류: {e}")

def check_and_rotate_log():
    global log_file_path, last_log_hour
    current_hour = datetime.now().hour
    is_midnight_rollover = (current_hour == 0 and last_log_hour == 23)
    is_forward_in_time = (current_hour > last_log_hour)
    if is_midnight_rollover or is_forward_in_time:
        today_str = time.strftime("%Y-%m-%d")
        LOG_DIRECTORY = os.path.join(BASE_LOG_DIR, today_str)
        if not os.path.isdir(LOG_DIRECTORY):
            os.makedirs(LOG_DIRECTORY)
        timestamp = time.strftime("%Y-%m-%d_%H-00-00")
        new_log_file_path = os.path.join(LOG_DIRECTORY, f"{timestamp}_system.log")
        log_file_path = new_log_file_path
        last_log_hour = current_hour
        log_message("INFO", f"새로운 시간대 로그 파일로 전환: {log_file_path}")

def log_message(level, message, data=None):
    check_and_rotate_log()
    if not log_file_path:
        return
    try:
        if level == "TX" and data is not None:
            output_hz_str = ", ".join([f"{hz:.1f}" for hz in data['output_hz_list']])
            timestamp = datetime.now().strftime('%Y%m%d %H:%M:%S')
            log_entry = f"{timestamp}, {data['input_hz']}, {data['pressure']:.1f}, {output_hz_str}\n"
        else:
            timestamp_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            log_entry = f"[{timestamp_str}] [{level}] {message}\n"
        with open(log_file_path, "a", encoding='utf-8') as f:
            f.write(log_entry)
    except Exception as e:
        print(f"로그 기록 오류: {e}")

SOH, STX, ETX, EOT, US = b'\x01', b'\x02', b'\x03', b'\x04', b'\x1F'
DIRECTION_SDC_TO_GW = b'\x30'
PRIMITIVE_DATA_SEND = b'2'
RS485_DE_PIN = 27

def calculate_checksum(packet_data: bytes) -> int:
    return sum(packet_data) & 0xFF

def encode_sensor_data_packet(pressure_val: float, hz_list: list, freq: int) -> bytes:
    try:
        hz_str = US.decode().join([f"{h:.1f}" for h in hz_list])
        data_payload_str = f"{pressure_val:.1f}{US.decode()}{hz_str}"
        data_payload_bytes = data_payload_str.encode('ascii')
        
        data_counter = (5).to_bytes(1, 'big')
        
        stx_to_etx = STX + data_counter + data_payload_bytes + ETX
        frequency_level = (freq & 0xFF).to_bytes(1, 'big')
        main_part = DIRECTION_SDC_TO_GW + PRIMITIVE_DATA_SEND + frequency_level + stx_to_etx
        
        length_field = (len(main_part) + 4).to_bytes(1, 'big')
        
        checksum_base = length_field + main_part + EOT
        checksum = (sum(checksum_base) & 0xFF).to_bytes(1, 'big')
        
        return SOH + checksum + length_field + main_part + EOT
    except Exception as e:
        print(f"인코딩 에러: {e}")
        return b''

GPIO.setmode(GPIO.BCM)
i2c = busio.I2C(board.SCL, board.SDA)
i2c_lock = threading.Lock()

ads = None
chan_pressure = None

try:
    ads = ADS.ADS1115(i2c, address=0x48)
    ads.data_rate = 860
    ads.gain = 1
    chan_pressure = AnalogIn(ads, 0)
except Exception as e:
        print(f"ADS1115 {hex(0x48)} 감지 실패")

MOSFET_CHANNELS = [17]
pwm_objects = {}
for pin in MOSFET_CHANNELS:
    try:
        GPIO.setup(pin, GPIO.OUT)
        pwm_objects[pin] = GPIO.PWM(pin, 10)
        pwm_objects[pin].start(0)
    except RuntimeError as e:
        print(f"PWM 설정 실패: GPIO {pin}, {e}")

GPIO.setup(RS485_DE_PIN, GPIO.OUT)
GPIO.output(RS485_DE_PIN, 0)

try:
    serial_port = serial.Serial("/dev/serial0", baudrate=9600, timeout=0.1)
    print("RS-485 포트 열기 성공")
except Exception as e:
    serial_port = None
    print(f"RS-485 포트 열기 실패: {e}")

ZERO_PRESSURE_VOLTAGE = 0.19825
VOLTAGE_PER_KPA = 0.03875 / 50

class FrequencyMonitor(threading.Thread):
    def __init__(self, sensor_obj):
        super().__init__()
        self.daemon = True
        self.sensor = sensor_obj
        self.running = True
        self.lock = threading.Lock()
        self._frequency = 0.0
        self.CURRENT_THRESHOLD = 80.0 
        self.SAMPLING_INTERVAL = 0.002

    def run(self):
        last_state_is_on = False
        last_rising_edge_time = time.time()
        
        try:
            with i2c_lock:
                self.sensor.set_calibration_32V_2A()
        except: pass
        
        while self.running:
            try:
                with i2c_lock:
                    measured_current = self.sensor.current
                
                current_state_is_on = measured_current > self.CURRENT_THRESHOLD
                now = time.time()
                
                if current_state_is_on and not last_state_is_on:
                    period = now - last_rising_edge_time
                    last_rising_edge_time = now
                    if period > 0.01:
                        freq = 1.0 / period
                        if 2 < freq < 60:
                            with self.lock:
                                self._frequency = freq
                
                if now - last_rising_edge_time > 0.5:
                    with self.lock:
                        self._frequency = 0.0
                
                last_state_is_on = current_state_is_on
                time.sleep(self.SAMPLING_INTERVAL)
            except Exception:
                time.sleep(0.1)

    def get_frequency(self):
        with self.lock:
            return self._frequency

    def stop(self):
        self.running = False

class ControlWindow(Gtk.Window):
    def __init__(self):
        super().__init__(title="Pulse Controller")
        self.set_default_size(800, 480)
        self.fullscreen()
        self.move(0, 0)
        self.set_decorated(False)
        self.fullscreen()
        self.run_thread = True
        self.motor_running = False
        self.active_freq = 10
        self.pending_freq = 10
        self.latest_pressure_kpa = 0.001
        self.output_frequencies = [0.0] * 4
        self.output_voltages = [0.0] * 4
        self.sensor_lock = threading.Lock()

        self.ina_sensors = {}
        self.freq_monitors = {}
        ina_addrs = {1: 0x40, 2: 0x41, 3: 0x44, 4: 0x45}
        for idx, addr in ina_addrs.items():
            try:
                sensor = INA219(i2c, addr)
                self.ina_sensors[idx] = sensor
                monitor = FrequencyMonitor(sensor)
                monitor.start()
                self.freq_monitors[idx] = monitor
            except:
                print(f"INA219 {hex(addr)} 감지 실패")

        self.sensor_reader_thread = threading.Thread(target=self.high_speed_sensor_reader, daemon=True)
        self.sensor_reader_thread.start()

        main_grid = Gtk.Grid(column_spacing=8, row_spacing=8, margin=8)
        main_grid.set_hexpand(True)
        main_grid.set_vexpand(True)
        main_grid.set_column_homogeneous(True)
        self.add(main_grid)

        self.create_display_area(main_grid)
        self.create_control_area(main_grid)
        self.create_status_area(main_grid)
        self.create_system_buttons(main_grid)
        self.apply_styles()

        GLib.timeout_add(100, self.update_gui)
        self.comm_thread = threading.Thread(target=self.communication_loop, daemon=True)
        self.comm_thread.start()

    def create_display_area(self, grid):
        self.display_labels = {}
        grid.set_column_homogeneous(False)
        grid.set_hexpand(True)
        square_height = 200
        self.display_labels['pressure_val'] = self.create_display_box("공급 압력", "0.001KPa")
        # self.display_labels['pressure_val'].set_size_request(100, square_height)
        grid.attach(self.display_labels['pressure_val'], 0, 0, 2, 1)
        self.display_labels['pulse_in_val'] = self.create_display_box("Pulse In", f"{self.pending_freq} Hz")
        # self.display_labels['pulse_in_val'].set_size_request(70, square_height)
        grid.attach(self.display_labels['pulse_in_val'], 2, 0, 1, 1)
        self.display_labels['pulse_out_vals'] = []
        for i in range(4):
            label = self.create_display_box(f"Pulse Out {i+1}", "0.0 Hz")
            # label.set_size_request(90, square_height)
            self.display_labels['pulse_out_vals'].append(label)
            grid.attach(label, 3 + i, 0, 1, 1)

    def create_display_box(self, title_text, value_text):
        box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
        box.set_name("display_box")
        
        box.set_hexpand(True)
        box.set_halign(Gtk.Align.FILL)
        
        title = Gtk.Label(label=title_text)
        title.set_name("display_title")
        value = Gtk.Label(label=value_text)
        value.set_name("display_value")
        box.pack_start(title, False, False, 5)
        box.pack_start(value, True, True, 5)
        return box

    def create_control_area(self, grid):
        control_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8, halign=Gtk.Align.CENTER)
        control_box.set_vexpand(True)
        control_box.set_hexpand(True)
        control_box.set_valign(Gtk.Align.CENTER)
        grid.attach(control_box, 0, 1, 9, 1)
        up_button = Gtk.Button(label="▲")
        up_button.connect("clicked", self.on_up_down_clicked, 1)
        up_button.set_name("control_button_arrow")
        up_button.set_hexpand(True)
        up_button.set_halign(Gtk.Align.FILL)
        down_button = Gtk.Button(label="▼")
        down_button.connect("clicked", self.on_up_down_clicked, -1)
        down_button.set_name("control_button_arrow")
        down_button.set_hexpand(True)
        down_button.set_halign(Gtk.Align.FILL)
        set_button = Gtk.Button(label="SET")
        set_button.connect("clicked", self.on_set_clicked)
        set_button.set_name("control_button_set")
        set_button.set_hexpand(True)
        set_button.set_halign(Gtk.Align.FILL)
        stop_button = Gtk.Button(label="STOP")
        stop_button.connect("clicked", self.on_stop_clicked)
        stop_button.set_name("control_button_stop")
        stop_button.set_hexpand(True)
        stop_button.set_halign(Gtk.Align.FILL)
        control_box.pack_start(up_button, True, True, 0)
        control_box.pack_start(down_button, True, True, 0)
        control_box.pack_start(set_button, True, True, 0)
        control_box.pack_start(stop_button, True, True, 0)

    def create_status_area(self, grid):
        self.channel_status_labels = []
        status_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0, halign=Gtk.Align.CENTER)
        status_box.set_name("status_bar")
        grid.attach(status_box, 0, 2, 9, 1)
        pressure_status = Gtk.Label(label="압력")
        pressure_status.set_name("status_item")
        status_box.pack_start(pressure_status, True, True, 0)
        for i in range(4):
            label = Gtk.Label(label=f"{i+1}")
            label.set_name("status_item")
            status_box.pack_start(label, True, True, 0)
            self.channel_status_labels.append(label)

    def create_system_buttons(self, grid):
        bottom_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, margin_top=10)
        bottom_box.set_valign(Gtk.Align.END)
        grid.attach(bottom_box, 0, 3, 9, 1)
        log_dir_button = Gtk.Button(label="로그파일")
        log_dir_button.connect("clicked", lambda w: subprocess.Popen(['xdg-open', BASE_LOG_DIR]))
        shutdown_button = Gtk.Button(label="시스템 종료")
        shutdown_button.connect("clicked", lambda w: os.system("sudo shutdown -h now"))
        bottom_box.pack_end(shutdown_button, False, False, 5)
        bottom_box.pack_end(log_dir_button, False, False, 5)

    def apply_styles(self):
        css_provider = Gtk.CssProvider()
        css_data = b"""
        window { background-color: #333; color: white; }
        #display_box { background-color: #111; border: 2px solid #555; border-radius: 8px; padding: 10px; }
        #display_title { font-size: 14pt; font-weight: bold; color: #AAA; }
        #display_value { font-size: 25pt; font-weight: bold; color: white; }
        #control_button_arrow { font-size: 45pt; min-height: 120px; border-radius: 12px;}
        #control_button_set { font-size: 35pt; font-weight: bold; background-color: #2196F3; color: white; min-height: 80px; }
        #control_button_stop { font-size: 35pt; font-weight: bold; background-color: #F44336; color: white; min-height: 80px; }
        #status_bar { background-color: #000; border-radius: 5px; }
        #status_item { font-size: 22pt; font-weight: bold; padding: 10px; min-width: 100px; }
        .connected { color: #4CAF50; }
        .disconnected { color: #F44336; }
        button { border-radius: 8px; }
        """
        css_provider.load_from_data(css_data)
        Gtk.StyleContext.add_provider_for_screen(Gdk.Screen.get_default(), css_provider, Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION)

    def on_up_down_clicked(self, button, step):
        self.pending_freq = max(5, min(40, self.pending_freq + step))
        log_message("INFO", f"주파수 조정 시도: {self.pending_freq} Hz")

    def on_set_clicked(self, button):
        self.active_freq = self.pending_freq
        self.motor_running = True
        for pwm in pwm_objects.values():
            pwm.ChangeFrequency(self.active_freq)
            pwm.ChangeDutyCycle(50)
        log_message("INFO", f"펌프 시작/주파수 설정: {self.active_freq} Hz")

    def on_stop_clicked(self, button):
        self.motor_running = False
        for pwm in pwm_objects.values():
            pwm.ChangeDutyCycle(0)
        log_message("INFO", "모든 펌프 정지")

    def high_speed_sensor_reader(self):
        gc.disable()
        while self.run_thread:
            pressure = self.read_pressure_sensor()
            
            freq_results = [0.0] * 4
            volt_results = [0.0] * 4
            for i in range(1, 5):
                if i in self.freq_monitors:
                    freq_results[i-1] = self.freq_monitors[i].get_frequency()
                if i in self.ina_sensors:
                    try:
                        with i2c_lock: volt_results[i-1] = self.ina_sensors[i].bus_voltage
                    except: pass

            with self.sensor_lock:
                if pressure is not None: self.latest_pressure_kpa = pressure
                self.output_frequencies = freq_results
                self.output_voltages = volt_results
            
            time.sleep(0.05)

    def read_pressure_sensor(self):
        if chan_pressure is None:
            return 0.001
        try:
            with i2c_lock:
                measured_voltage = chan_pressure.voltage
            pressure_kpa = (measured_voltage - ZERO_PRESSURE_VOLTAGE) / VOLTAGE_PER_KPA
            return max(0.001, pressure_kpa)
        except Exception:
            return 0.001

    def update_gui(self):
        if not self.run_thread:
            return False
        with self.sensor_lock:
            p = self.latest_pressure_kpa
            freqs = list(self.output_frequencies)
            self.display_labels['pressure_val'].get_children()[1].set_text(f"{p:.1f} kPa")
            self.display_labels['pulse_in_val'].get_children()[1].set_text(f"{self.pending_freq} Hz")
            for i in range(4):
                txt = f"{freqs[i]:.1f}Hz"
                self.display_labels['pulse_out_vals'][i].get_children()[1].set_text(txt)
                
                is_conn = not (self.motor_running and freqs[i] < 1.0)
                self.set_label_connection_style(self.channel_status_labels[i], is_conn)
        return True

    def set_label_connection_style(self, label, is_connected):
        context = label.get_style_context()
        if is_connected:
            context.remove_class("disconnected"); context.add_class("connected")
        else:
            context.remove_class("connected"); context.add_class("disconnected")

    def communication_loop(self):
        while self.run_thread:
            if self.motor_running and serial_port is not None:
                start_time = time.time()
                try:
                    with self.sensor_lock:
                        p = self.latest_pressure_kpa
                        fs = list(self.output_frequencies)
                        in_hz = self.active_freq
                    
                    packet = encode_sensor_data_packet(p, fs, in_hz)
                    
                    if packet:
                        GPIO.output(RS485_DE_PIN, 1)
                        time.sleep(0.002)
                        serial_port.write(packet)
                        serial_port.flush()
                        
                        tx_wait = (len(packet) * 11 / 9600) + 0.01
                        time.sleep(tx_wait)
                        GPIO.output(RS485_DE_PIN, 0)
                    
                    log_data = {'input_hz': in_hz, 'pressure': p, 'output_hz_list': fs}
                    log_message("TX", "RS-485", data=log_data)
                except Exception as e:
                    print(f"RS-485 오류: {e}")
                
                target = 1.0 / self.active_freq
                time.sleep(max(0.01, target - (time.time() - start_time)))
            else:
                time.sleep(0.1)

    def on_destroy(self, *args):
        self.run_thread = False
        for monitor in self.freq_monitors.values():
            monitor.stop()
        time.sleep(0.2)
        for pwm in pwm_objects.values(): pwm.stop()
        if serial_port: serial_port.close()
        GPIO.cleanup()
        Gtk.main_quit()

if __name__ == "__main__":
    try: os.nice(-20)
    except: pass
    import signal
    signal.signal(signal.SIGINT, signal.SIG_DFL)
    setup_logging()
    win = ControlWindow()
    win.connect("destroy", win.on_destroy)
    win.show_all()
    Gtk.main()
